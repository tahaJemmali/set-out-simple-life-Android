...
public class NameActivity extends AppCompatActivity implements IRepository {
    ...
    public void saveBtn(View view){
        ...
        if (!CustomSettingsObjectEditText.firstNameText.matches(namePattern))
        {
            Toast.makeText(NameActivity.this,"Invalid first name!",Toast.LENGTH_SHORT).show();
            return;
        }

        if (!CustomSettingsObjectEditText.lastNameText.matches(namePattern))
        {
            Toast.makeText(NameActivity.this,"Invalid last name!",Toast.LENGTH_SHORT).show();
            return;
        }

        User u = new User();
        u.setEmail(HomeActivity.getCurrentLoggedInUser().getEmail());
        u.setFirstName(CustomSettingsObjectEditText.firstNameText);
        u.setLastName(CustomSettingsObjectEditText.lastNameText);
        UserRepository.getInstance().updateUser(u, NameActivity.this);

        //currentUser update
        HomeActivity.getCurrentLoggedInUser().setFirstName(CustomSettingsObjectEditText.firstNameText);
        HomeActivity.getCurrentLoggedInUser().setLastName(CustomSettingsObjectEditText.lastNameText);
